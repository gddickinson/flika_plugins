...
class OnTheFly(BaseProcess_noPriorWindow):
    ...
    def __call__(self, recordingFolder,exportFolder, batchSize, updateRate, updateRateVolViewer, nSteps, displaySlice, shift_factor, theta, triangle_scan, interpolate, trim_last_frame, zscan, nChannels, keepSourceWindow=False):
...
class MainGUI(QtWidgets.QDialog):
    def __init__(self, parent = None):
        ...
        self.startVolumeViewer_button = QtWidgets.QPushButton('Run Batch Analysis Now')
        #self.startVolumeViewer_button.pressed.connect(self.sendBatchToVolumeAnalyser)
    ...
    def addVolumeToArray(self, filename):
        ...
        if self.recordingArray_FLAG != False: #continue adding frames to recording array
            ...
            #perform volume analysis -> saves transformed arrays to disk
            self.sendVolToVolumeAnalyser(newVol_1, self.numberVolsAdded_channel1)
    ...
    def sendVolToVolumeAnalyser(self, vol, volNumber):
        ...
        #send to volume analyzer
        #self.volumeViewer = Volume_Viewer(w)
        
        volumeAnalyzer.run(D,self.exportFolderPath,volNumber)
        return 
