        ...
        try:
            ...
            if self.trackPlotOptions.trackColour_checkbox.isChecked():
                ...
                if color_mode in track.columns:
                    ...
                    logger.debug(f"  Set pen color from {color_mode}")

            pathitem.setPen(pen)
            logger.debug(f"  Pen set: color={pen.color().name()}, width={pen.width()}, cosmetic={pen.isCosmetic()}")
            
            self.plotWindow.imageview.view.addItem(pathitem)
            logger.debug(f"  Added pathitem to view")
            ...
            if hasattr(self, 'overlayWindow'):
                ...
                logger.debug(f"  Added overlay path")
            ...
            logger.debug(f"  Coordinates: {len(x)} points, x range=({x.min():.2f}, {x.max():.2f}), y range=({y.min():.2f}, {y.max():.2f})")

            if len(x) < 2:
                logger.warning(f"  Track {track_id} has <2 points, skipping")
            ...
            logger.debug(f"  QPainterPath created: boundingRect={path.boundingRect()}")

            pathitem.setPath(path)
            logger.debug(f"  Path set to pathitem, visible={pathitem.isVisible()}, opacity={pathitem.opacity()}")
