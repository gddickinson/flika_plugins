...
class VolumeSlider(BaseProcess):
    ...
    def getVolumes(self):
        ...
        #get volume files in folder and subdirectories
        vols = list()
        for (dirpath, dirnames, filenames) in os.walk(self.volume_path):
            vols += [os.path.join(dirpath, file) for file in filenames]
        #add volumes to volume list
        for vol in vols:
            self.A_list.append(self.openTiff(vol))
        self.numberOfTimeSlices = (len(self.A_list))
        self.volsAdded = vols
        self.volsAddedFlag = True
        return
    ...
    def updateVolume(self):
        if self.volsAddedFlag == False:
            print('Viewer Not Initiated')
            ...
            #add volumes to volume list
            for vol in vols:
                if vol not in self.volsAdded:
                    self.A_list.append(self.openTiff(vol))
            
            self.numberOfTimeSlices = (len(self.A_list))
            self.volsAdded.append(vols)
