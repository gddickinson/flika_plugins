...
def save_localizations_csv(localizations, filepath, include_metadata=True,
    ...
    flika_compatible : bool
        If True (default), swap x/y to match ImageJ thunderSTORM/FLIKA convention
        where x=vertical (row), y=horizontal (column) for FLIKA display compatibility.
        Set to False for standard coordinate convention.

    Notes
    -----
    FLIKA's coordinate system expects [vertical, horizontal] for scatter points.
    ImageJ thunderSTORM saves with swapped coordinates to match this convention.
    When flika_compatible=True, this function matches ImageJ's format.
    """
    # Convert to DataFrame
    data = {}

    # Core columns - swap x/y if FLIKA compatible
    if flika_compatible:
        # Match ImageJ thunderSTORM format for FLIKA display
        # FLIKA expects: x=vertical (row), y=horizontal (column)
        if 'y' in localizations:  # Our y (vertical) → CSV x column
            data['x [nm]'] = localizations['y']
        if 'x' in localizations:  # Our x (horizontal) → CSV y column
            data['y [nm]'] = localizations['x']
    else:
        # Standard convention: x=horizontal, y=vertical
