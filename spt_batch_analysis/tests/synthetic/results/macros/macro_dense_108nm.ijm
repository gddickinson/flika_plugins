...
run("Export results", "filepath=[/Users/george/claude_test/spt_batch_analysis/tests/synthetic/results/imagej_results/dense_108nm_imagej.csv] fileformat=[CSV (comma separated)] sigma=true intensity=true chi2=true offset=true saveprotocol=true x=true y=true bkgstd=true uncertainty=true frame=true id=true");
f = File.open("/Users/george/claude_test/spt_batch_analysis/tests/synthetic/results/imagej_results/dense_108nm_imagej_timing.txt");
print(f, "dense_108nm," + elapsed_ms);
File.close(f);
while (nImages>0) {
    selectImage(nImages);
    close();
}
print("Test dense_108nm complete.");
